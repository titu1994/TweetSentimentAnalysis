...
def gen_multiplicative_lstm_model():
    ...
    with tf.name_scope('mlstm'):
        x = MultiplicativeLSTM(512, dropout_W=0.2, dropout_U=0.2)(embedded_sequences)
...
def write_predictions(model_dir='multiplicative_lstm/', mode='train', dataset='full'):
    ...
    model = gen_multiplicative_lstm_model()
    ...
    if mode == 'train':
        np.save(basepath + "multiplicative_lstm_predictions.npy", model_predictions)
    else:
        if dataset == 'full':
            save_dir = 'test'
        else:
        ...
        preds_save_path = save_dir + "/" + model_dir + "multiplicative_lstm_predictions.npy"
        np.save(preds_save_path, model_predictions)


def calculate_score(model_dir='multiplicative_lstm/', base_dir='test/', dataset='full'):
...
if __name__ == '__main__':
    gen_multiplicative_lstm_model()
    train_keras_model_cv(gen_multiplicative_lstm_model, 'multiplicative_lstm/multiplicative-lstm-model', max_nb_words=MAX_NB_WORDS,
